  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ getValue, row }) => {
        ...
        return (
          <span
            className={`don-badge don-badge--${tipo?.toLowerCase()} ${
              estado === 'anulada' ? 'don-badge--anulada' : ''
            }`}
          >
      ...
      cell: ({ row }) => (
        <div className="don-table-actions">
          ...
          {row.original?.estado === 'anulada' ? (
            ...
            <button
              className="don-btn don-btn--danger don-btn--sm"
              onClick={() => onAnular(row.index)}
            >
        ...
        <tbody>
          {table.getRowModel().rows.length === 0 ? (
            ...
            table.getRowModel().rows.map((row) => (
              <tr
                key={row.id}
                className={row.original?.estado === 'anulada' ? 'don-row--anulada' : ''}
              >
