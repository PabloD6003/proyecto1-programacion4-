...
const TablaDonaciones = ({ donaciones, donacionesCompletas, onAnular, onEditar }) => {
  const getIndiceReal = (donacion) => donacionesCompletas.indexOf(donacion)
  ...
  const columns = [
    ...
    },
    {
      header: 'Acciones',
      cell: ({ row }) => {
        const indiceReal = getIndiceReal(row.original)
        return (
          <div className="don-table-actions">
            <button
              className="don-btn don-btn--outline don-btn--sm"
              onClick={() => onEditar(indiceReal, row.original)}
            >
